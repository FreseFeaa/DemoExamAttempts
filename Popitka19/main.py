...
def post_register(request: Request,
    ...
    if not ree.fullmatch(r'[a-zA-Z0-9]{6,}', login):
        error = "Логин: Латиница + цифры, больше 6 символов"
    elif len(password) < 8:
...
def get_admin(request: Request, status: str | None = Query(None)):
    ...
    with get_db() as conn:
        params = []
        base = "select requests.*, users.login, users.fio from requests, users where users.id = requests.user_id"

        if status and status != "Все":
            base += " AND requests.status = ?"
            params.append(status)
            
        request_list = conn.execute(base, params).fetchall()

    return templates.TemplateResponse("admin.html", {"request": request, "requests": request_list, "cur_status": status or "Все"})
