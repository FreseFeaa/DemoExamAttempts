...
COURSES = ["База данных", "Алгоритмы", "Роблокс"]
PAYMNETS = ["Наличные","Перевод по номеру"]
...
def get_create_request(request: Request):
    ...
    return templates.TemplateResponse("create_request.html",{"request":request, "courses": COURSES, "payments": PAYMNETS})
...
def get_create_request(request: Request,
    ...
    with get_db() as conn:
        conn.execute("insert into requests (course_name,payment_method,date_start,user_id) values (?,?,?,?)", (course,payment,date,user_id))
    
    return RedirectResponse(url="/profile", status_code=302)

@app.post("/add_review")
def post_profile(request: Request,
                 request_id: str = Form(...),
                 review: str = Form(...)):
    user_id = request.session.get("user_id")
    if not user_id:
