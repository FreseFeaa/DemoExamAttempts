...
# DATABASE_URL = "postgresql://postgres:123@localhost:5432/Korok"
...
def get_admin(request: Request, status: str | None = Query(None)):
    ...
    base = "select requests.*, users.fio, users.login from requests, users where requests.user_id = users.id "
    params = []
