...
def get_profile(request: Request):
    ...
    with get_db() as conn:
        request_id = conn.execute("select * from requests where user_id = ?", (user_id,)).fetchall()

    return templates.TemplateResponse("profile.html", {"request": request, "requests": request_id})
...
def get_admin(request: Request, status: str | None = Query(None)):
    ...
    with get_db() as conn:
        request_id = conn.execute(base,params).fetchall()

    return templates.TemplateResponse("admin.html", {"request": request, "requests": request_id, "cur_status": status or 'Все'})
