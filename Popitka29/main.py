...
@app.get("/admin")
def get_profile(request: Request, status: str | None = Query(None)):
...
def post_add_review(request: Request,
    ...
    with get_db() as conn:
        cur = conn.execute("select * from requests where id = ? and review is null and status = 'Обучение завершено'",(request_id,)).fetchone()
        if cur:
