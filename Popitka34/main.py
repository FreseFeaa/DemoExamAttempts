...
from fastapi import Request, Query, Form, FastAPI
...
def post_add_review(request: Request,
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("select * from requests where id = %s and status = 'Обучение завершено' and reveiw is null", (request_id,))
        data = cur.fetchall()
        if data:
            cur.execute("update requests set reveiw = %s where id = %s", (review, request_id))
            conn.commit()
        return RedirectResponse("/profile", status_code=302)
