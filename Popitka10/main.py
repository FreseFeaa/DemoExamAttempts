...
def post_login(request: Request,
    ...
    if login == "Admin":
        if password == "KorokNET":
            ...
            return templates.TemplateResponse("login.html", {"request":request, "error": "Данные неверные"})
...
def post_add_review(request: Request,
    ...
    with get_db() as conn:
        cur = conn.execute("select * from requests where user_id = ? and status = 'Обучение завершено' and review is null", (user_id,))
        if cur:
