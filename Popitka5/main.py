...
COURSES = ["Курс алгоритмов", "Курс баз данных", "Курс оригами"]
PAYMENTS = ["Наличка","Перевод по номеру"]
...
def get_admin(request: Request):
    ...
    with get_db() as conn:         
        request_list = conn.execute("select requests.*, users.id, users.login from requests, users where requests.user_id = users.id").fetchall()
