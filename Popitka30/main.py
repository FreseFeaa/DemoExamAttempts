...
def init_db():
    ...
    create table if not exists requests(
                           id integer primary key autoincrement,
                           user_id inteher references users(id) on delete cascade,
...
def get_admin(request: Request, status: str | None = Query(None)):
    ...
    with get_db() as conn:
        requests_list = conn.execute(base, param ).fetchall()
    return templates.TemplateResponse("admin.html", {"request":request, "requests": requests_list, "cur_status": status})
